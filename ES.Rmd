...
\begin{equation} 
Hedge's \; g_s = Cohen's \; d_s \times \frac{\Gamma(\frac{df_{Student}}{2})}{\sqrt{\frac{df_{Student}}{2}} \times \Gamma(\frac{df_{Student}-1}{2})}
(\#eq:Hedgesgs)
\end{equation} 
...
Hedge's $g_s$ is theoretically unbiased when the assumptions of normality and equal variances are met (see Table 1), and it has a smaller variance than Cohen's $d_s$, especially with small sample sizes \footnote{$.52 \le \left[\frac{\Gamma(\frac{df}{2})}{\sqrt{\frac{df}{2}} \times \Gamma(\frac{df-1}{2})} \right] ^2 < 1$ for $3 \le df < \infty$. The larger the total sample size, the smaller the difference between the variance of Cohen's $d_s$ and Hedge's $g_s$.}. As Cohen's $d_s$, its variance depends on the total sample size (N), the sample sizes ratio $\left( \frac{n_1}{n_2}\right)$ and the population effect size ($\delta_{Cohen}$). How these parameters influence the variance of Hedge's $g_s$ will be summarized in a later section in which we will compare different estimators through Monte Carlo simulations (see the section "Monte Carlo Simulations").
...
*Note*. Cohen's $d_s$ is a biased estimator, because its expectation differ from the population effect size. On the other hand, Hedge's $g_s$ is an unbiased estimator, because its expectation equals the population effect size; equations in Table 1 require $df \ge 3$ (i.e. $N \ge 5$).

\elandscape

## When variances are unequal between populations

In his review, @Shieh_2013 mentions three options available in the literature to deal with the case of unequal variances: the sample mean difference divided by (A) the Glass's $d_s$, (B) the non pooled average of both variance estimates and (C) the Shieh's $d_s$. 
...
### Glass's $g_s$, Cohen's $g'_s$ and Shieh's $g_s$

As for Cohen's $d_s$, an Hedge's correction can be applied in order to compensate for the bias of Glass's $d_s$, Cohen's $d'_s$ and Shieh's $d_s$ with small sample sizes (see Table 2). This correction has the following general form:
...
Est-ce mieux de faire 4 tables distinctes, ou les 2 qui suivent?
...
| Estimator|   Bias       |               Variance                     |      df              | 
|--------|:--------------:  | :------------------------------------------:  | :------------------------------------------:  |
| \tiny$Hedges's \; g_s$    |/|  \tiny$Var(Cohen's \; d_s) \times \left[ \frac{\Gamma(\frac{df}{2})}{\sqrt{\frac{df}{2}} \times \Gamma(\frac{df-1}{2})} \right]^2$   | \tiny$N-2$|
|     |       |  \tiny$\approx Var(Cohen's \; d_s) \times \left[1-\frac{3}{4N-9}\right]^2$   ||
|    |      |  |    |
| \tiny$Glass's \; g_s$    |/|\tiny$Var(Glass's \; d_s) \times \left[\frac{\Gamma(\frac{df}{2})}{\sqrt{\frac{df}{2}} \times \Gamma(\frac{df-1}{2})}\right]^2$ |   \tiny$n_c-1$  |
|    |      |  |    |
| \tiny$Cohen's \; g'_s$    |/|\tiny$Var(Cohen's \; d'_s) \times \left[\frac{\Gamma(\frac{df}{2})}{\sqrt{\frac{df}{2}} \times \Gamma(\frac{df-1}{2})}\right]^2$ |\tiny$\frac{(n_1-1)(n_2-1)(s^2_1+s^2_2)^2}{(n_2-1)s^4_1+(n_1-1)s^4_2}$     |
|    |      |  |    |
|  \tiny$Shieh's \; g_s$  | / |\tiny$Var(Shieh's \; d_s) \times \left[\frac{\Gamma(\frac{df}{2})}{\sqrt{\frac{df}{2}} \times \Gamma(\frac{df-1}{2})}\right]^2$    |\tiny$\approx \frac{\left(\frac{\sigma^2_1}{n_1}+\frac{\sigma^2_2}{n_2} \right)^2}{\frac{(\sigma^2_1/n_1)^2}{n_1-1}+\frac{(\sigma^2_2/n_2)^2}{n_2-1}}$|

Table: Theoretical bias and variance of unbiased estimators.

*Note*. Equations for bias and variance of Cohen's $d_s$ require that i.i.d. residuals are normally distributed ($\epsilon_i \sim i.i.d.\; N(0,\sigma^2)$)   with equal variances across gruops ($\sigma_1=\sigma_2$). Equations for bias and variance of all other estimators require only that i.i.d. residuals are normally distributed.
