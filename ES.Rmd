...
|     |\tiny$\approx \frac{\delta_{Cohen}}{\left(1-\frac{3}{4(n_1+n_2)-9}\right)}$|\tiny$\approx \delta_{Cohen} \left[\frac{1}{\left(1-\frac{3}{4(n_1+n_2)-9}\right)}-1\right]$ | \tiny$\frac{N-2}{(N-4) \times \frac{n_1n_2}{N}} \times \left(1+\frac{n_1n_2}{N} \times \delta_{cohen}^2\right) -\delta_{cohen}^2 \times \left[\frac{1}{\left(1-\frac{3}{4(n_1+n_2)-9}\right)}\right]^2$    |
|    |      |  |    |
...
|    |      |  |    |
|  \tiny$Shieh's \; d_s$  | \tiny$\delta_{Shieh} \times \frac{\sqrt{\frac{df}{2}} \times \Gamma(\frac{df-1}{2})}{\Gamma(\frac{df}{2})}$  | \tiny$\delta_{Shieh} \times \left(\frac{\sqrt{\frac{df}{2}} \times \Gamma(\frac{df-1}{2})}{\Gamma(\frac{df}{2})}-1 \right)$ |    |
|    |      |  |    |
| ||    \tiny$with \; df=\frac{\left(\frac{SD^2_1}{n_1}+\frac{SD^2_2}{n_2} \right)^2}{\frac{(SD^2_1/n_1)^2}{n_1-1}+\frac{(SD^2_2/n_2)^2}{n_2-1}}$              |
*Note.* N is the total sample size (n1+n2). Equations to compute expectency, bias and variances of Cohen's ds, Hedge's gs and Glass's ds require the assumptions of normality and equal variances.
