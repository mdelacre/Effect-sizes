...
meandiff.CI <- function(Group.1, Group.2,conf.level=.95,var.equal=FALSE,alternative){ # alternative="two.sided", greater or "less"
  n1 <- length(Group.1)
  n2 <- length(Group.2)
  mean1 <- mean(Group.1)
  mean2 <- mean(Group.2)
  ...
  if(alternative=="two.sided"){
    ...
    if (var.equal==TRUE){
      ...
      f=function(theo_mudiff,rep) 1-pt(q=((mean1-mean2)-(theo_mudiff))/SE, df=n1+n2-2)-rep
      out=uniroot(f,c(-100,100),rep=1-conf.level)
      theo_mudiff.1 <- out$root
...
Group.1 <- c(8,5,3,2,5)
Group.2 <- c(1,3,4,5,8)
round(meandiff.CI(Group.1, Group.2,conf.level=.90,var.equal=FALSE,alternative="two.sided"),3)
...
Cohen.CI <- function(Group.1, Group.2,conf.level,alternative="two.sided") #alternative = "two.sided", "less" or "greater"
{ 
  n1 <- length(Group.1)
  n2 <- length(Group.2)
...
Group.1 <- c(8,5,3,2,5)
Group.2 <- c(1,3,4,5,8)
round(Cohen.CI(Group.1, Group.2,conf.level=.95,alternative="greater"),4)
...
Group.1 <- c(8,5,3,2,5)
Group.2 <- c(1,3,4,5,8)
round(Cohen.CI(Group.1, Group.2,conf.level=.95,alternative="two.sided"),4)
