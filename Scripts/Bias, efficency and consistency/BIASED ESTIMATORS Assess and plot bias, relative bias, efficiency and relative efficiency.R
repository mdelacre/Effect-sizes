...
#setwd("D:/Documents/Github_projects/Effect-sizes/Scripts outputs")
...
for (i in seq_len(length(Folder))){
  ...
  estimator <- c("Cohen","Glass1","Glass2","Shieh", "cohen_delta_prime")
...
releff_cohen_delta_prime <- var(file[,17])/cohen_delta_prime^2
    ...
    setwd("D:/Documents/Github_projects/Effect-sizes/Scripts outputs/Quality of ES measures/Data summary/Biased estimators")
...
# legend("center", 
#       legend=c(expression(paste("Cohen's ",g[s])),expression(paste("Glass's ",g[s],"(",sigma," =",S[1],")")),expression(paste("Glass's ",g[s],"(",sigma," =",S[2],")")),expression(paste("Shieh's ",g[s])),
...
for (j in seq_len(length(list.files(Path)))){
  ...
  for (i in seq_len(length(Conditions_id))){ 
    ...
    res <- matrix(0,9,7)  
    names<-expand.grid("bias_",c("Cohen","Glass1","Glass2","Shieh","cohen_delta_prime"))
    ...
    res3 <- matrix(0,9,7)  
    names<-expand.grid("var_",c("Cohen","Glass1","Glass2","Shieh","cohen_delta_prime"))
...
for (j in seq_len(length(list.files(Path)))){
  ...
  res[,1] <- sdval
  res[,2] <- tapply(Sel$bias_Cohen,list(Sel$sd1.sd2),mean)[1:6]
  ...
  res3[,1] <- sdval 
  res3[,2] <- tapply(Sel$eff_Cohen,list(Sel$sd1.sd2),mean)[1:6]
  ...
  setwd(paste0("D:/Documents/Github_projects/Effect-sizes/Scripts outputs/Quality of ES measures/Graphs/Biased estimators/Raw estimators of goodness/id_Het_bal/sd-ratio"))
  if(G1==-2.08){
    g1=2.08
  } else {g1=G1}
  png(file=paste0("bias_eff,G1=",g1, " & G2=",G2,";",names(Conditions_id)[i], ".png"),width=1400,height=1700, units = "px", res = 300)  
...
plot_hetr <- function(totalN){for (j in seq_len(length(list.files(Path)))){
  ...
  res[,4] <- tapply(Sel$bias_Cohen,list(Sel$sd1.sd2,Sel$n1,Sel$n2),mean)[1:6]
  ...
  res3[,4] <- tapply(Sel$eff_Cohen,list(Sel$sd1.sd2,Sel$n1,Sel$n2),mean)[1:6]
...
for (j in seq_len(length(list.files(Path)))){
  ...
  for (i in seq_len(length(Conditions_id))){ 
    ...
    setwd("D:/Documents/Github_projects/Effect-sizes/Scripts outputs/Quality of ES measures/Graphs/Biased estimators/Relative estimators of goodness/")
    #dir.create(names(Conditions_id)[i])
    
    setwd(paste0("D:/Documents/Github_projects/Effect-sizes/Scripts outputs/Quality of ES measures/Graphs/Biased estimators/Relative estimators of goodness/",names(Conditions_id)[i]))
    if(G1==-2.08){
      g1=2.08
    } else {g1=G1}
    png(file=paste0("bias_eff,G1=",g1, " & G2=",G2,";",names(Conditions_id)[i], ".png"),width=1400,height=1700, units = "px", res = 300)  
...
for (j in seq_len(length(list.files(Path)))){
  ...
  colnames(res) <- c("sd-ratio",paste0(names[,1],names[,2]))
  res[,1] <- sdval
  res[,2] <- tapply(Sel$relbias_Cohen,list(Sel$sd1.sd2),mean)[1:6]
  ...
  colnames(res3) <- c("sd-ratio",paste0(names[,1],names[,2]))
  res3[,1] <- sdval 
  res3[,2] <- tapply(Sel$releff_Cohen,list(Sel$sd1.sd2),mean)[1:6]
  ...
  setwd(paste0("D:/Documents/Github_projects/Effect-sizes/Scripts outputs/Quality of ES measures/Graphs/Biased estimators/Relative estimators of goodness/id_Het_bal/sd-ratio"))
  if(G1==-2.08){
    g1=2.08
  } else {g1=G1}
...
plot_hetr <- function(totalN){for (j in seq_len(length(list.files(Path)))){
  ...
  for (i in seq_len(length(Conditions_id))){ 
    ...
    colnames(res) <- c("n1","n2","sd-ratio",paste0(names[,1],names[,2]))
    res[,1:3] <- cbind(combi[,2],combi[,3],combi[,1])
    res[,4] <- tapply(Sel$relbias_Cohen,list(Sel$sd1.sd2,Sel$n1,Sel$n2),mean)[1:6]
    ...
    # Matrix containing variances information
    res3 <- matrix(0,6,8)  
    names<-expand.grid("eff_",c("Cohen","Glass1","Glass2","Shieh","cohen_delta_prime"))
    colnames(res3) <- c("n1","n2","sd-ratio",paste0(names[,1],names[,2]))
    res3[,1:3] <- cbind(combi[,2],combi[,3],combi[,1])
    res3[,4] <- tapply(Sel$releff_Cohen,list(Sel$sd1.sd2,Sel$n1,Sel$n2),mean)[1:6]
...
plot_hetr <- function(ratio){for (j in seq_len(length(list.files(Path)))){
  ...
  for (i in seq_len(length(Conditions_id))){ 
    ...
    res[,5] <- tapply(Sel$relbias_Cohen,list(Sel$n1.n2,Sel$sd1.sd2),mean)[1:6]
    ...
    res[,9] <- tapply(Sel$relbias_Cohen,list(Sel$n1.n2,Sel$sd1.sd2),mean)[1:6]
    
    # Select only bias columns
    res.relbias <- t(res[,5:9])
    colnames(res.relbias) <- paste0("1:",res[,2],"\n (",res[,3],":",res[,4],")")

    setwd("D:/Documents/Github_projects/Effect-sizes/Réflexion-extras/interaction_n_sd_shieh&Cohen")
    ...
    res3[,5] <- tapply(Sel$releff_Cohen,list(Sel$n1.n2,Sel$sd1.sd2),mean)[1:6]
