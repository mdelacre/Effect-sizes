...
for (i in seq_len(length(Folder))){
  ...
  good_mes <-matrix(0,length(list.files(Folder[i])),5+2*6)
  goodness_indic <- c("bias_","relbias_")
  ...
  write.table(good_mes,paste0(sub,",relative bias.txt"),sep=";",dec=",")
...
for (j in seq_len(length(list.files(Path,pattern = ".*relative bias.txt")))){
  

  File=read.table(paste0(Path,list.files(Path,pattern = ".*relative bias.txt")[j]),header=T,sep=";",dec=",")  
  ...
  # We also need to exclude simulations where mu1-mu2=0 (on cannot compute relative bias when mu1-mu2=0...)
  ...
  for (i in seq_len(length(Conditions_id))){ 
    ...
    # Matrix containing relative bias information
    res2 <- matrix(0,9,8)  
    ...
    param <- str_extract_all(list.files(Path,pattern=".*relative bias.txt")[j], "[[:digit:]]+\\.*[[:digit:]]*")
    ...
    png(file=paste0("relativebias,G1=",G1, " & G2=",G2,";",names(Conditions_id)[i], ".png"),width=1000,height=1700, units = "px", res = 300)  
    
    par(xpd = F,mar = c(4,5,1.5,0),mfrow = c(2,1))   
    ...
    if (j==1){ylabeleff=expression(paste("[E(" , hat(delta) , ") -",delta,"]/",delta ))
    } else {ylabeleff=""}
