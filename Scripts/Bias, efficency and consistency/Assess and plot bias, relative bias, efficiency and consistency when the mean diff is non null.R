...
for (i in seq_len(length(Folder))){
  ...
  goodness_indic <- c("bias_","relbias_","eff_","MSE_")
  ...
  colnames(good_mes) <- c("n1","n2","n1/n2","m1-m2","sd1/sd2",paste0(columns_names[,2],columns_names[,1]))
    ...
    good_mes[j,24] <- MSE_cohen
    good_mes[j,25] <- MSE_hedge
    good_mes[j,26] <- MSE_glass1
    good_mes[j,27] <- MSE_glass2
    good_mes[j,28] <- MSE_shieh
    good_mes[j,29] <- MSE_shieh_corr
...
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------
#     GRAPH: PLOTTING bias, relative bias, EFFICIENCY and MSE
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
for (j in seq_len(length(list.files(Path,pattern = ".*good_mes.txt")))){
  ...
  for (i in seq_len(length(Conditions_id))){ 
    ...
    res4[,3] <- tapply(Sel$relbias_Cohen,list(Sel$n1,Sel$n2),mean)[1:9]
    res4[,4] <- tapply(Sel$relbias_Hedge,list(Sel$n1,Sel$n2),mean)[1:9]
    res4[,5] <- tapply(Sel$relbias_Glass1,list(Sel$n1,Sel$n2),mean)[1:9]
    res4[,6] <- tapply(Sel$relbias_Glass2,list(Sel$n1,Sel$n2),mean)[1:9]
    res4[,7] <- tapply(Sel$relbias_Shieh,list(Sel$n1,Sel$n2),mean)[1:9]
    res4[,8] <- tapply(Sel$relbias_Shieh_corr,list(Sel$n1,Sel$n2),mean)[1:9]
    # Select only rows with no "NA"  
    res4 <- subset(res4,res4[,3] != "NA") 
    # Select only bias columns
    res.relbias <- t(res4[,3:8])
