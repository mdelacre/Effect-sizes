...
For all ``biased'' estimators, \(\delta_{bias}=\delta \times (c_f-1)\) where \(c_f=\frac{\sqrt{\frac{df}{2}} \times \Gamma\left(\frac{df-1}{2}\right)}{\Gamma\left( \frac{df}{2}\right)}\) and the distinct values of \(df\) are provided in the main article, in equation 7 for Cohen's \(d\), in equation 9 for Glass' \(d\), in equation 11 for Shieh's \(d\) and in equation 16 for Cohen's \(d^*\). As we can see in the equation of \(\delta_{bias}\), when the population effect size is null so is the bias. We will therefore focus on configurations where there is a non-null population effect size. The sampling distribution of Cohen's \(d\) (and therefore its bias) is only known under the assumptions of normality and homoscedasticity. On the other hand, the biases of Glass' \(d\), Cohen's \(d^*\) and Shieh's \(d\) are theoretically known for all configurations where the normality assumption is met. In order to simplify the analysis of their bias, it is convenient to subdivide all configurations into 3 conditions:\\
...
For all previously mentioned estimators (Cohen's \(d\), Glass' \(d\), Cohen's \(d^*\) and Shieh's \(d\)), the theoretical expectation is computed by multiplying the population effect size (respectively \(\delta_{Cohen}\), \(\delta_{Glass}\), \(\delta^*_{Cohen}\) and \(\delta_{Shieh}\)) by the following multiplier coefficient:
\begin{equation} 
\gamma=\frac{\sqrt{\frac{df}{2}} \times \Gamma{\left(\frac{df-1}{2}\right)}}{\Gamma{\left(\frac{df}{2}\right)}}
\label{eq:mc}
\end{equation}

where \(df\) are the degrees of freedom (see the main article). One can show that \(\gamma\) is always bigger than 1, meaning that when the population effect size is not zero, all estimators will overestimate the population effect size. Moreover, its limit tends to 1 when the degrees of freedom (\(df\)) tend to infinity, meaning that the larger the degrees of freedom, the lower the bias.\\
...
\hypertarget{cohens-bmd-see-table-1}{%
\subsection{\texorpdfstring{Cohen's \(\bm{d}\) (see Table 1)}{Cohen's \textbackslash bm\{d\} (see Table 1)}}\label{cohens-bmd-see-table-1}}

Under the assumptions that independent residuals are normally distributed with equal variances, the \textbf{bias} of Cohen's \(d\) is a function of total sample size (\(N\)) and the population effect size (\(\delta_{Cohen}\)):
...
\hypertarget{glass-bmd-see-table-2}{%
\subsection{\texorpdfstring{Glass' \(\bm{d}\) (see Table 2)}{Glass' \textbackslash bm\{d\} (see Table 2)}}\label{glass-bmd-see-table-2}}
...
The \textbf{bias} of Glass' \(d\) is a function of the control group size (\(n_c\)) and the population effect size (\(\delta_{Glass}\)):

\begin{itemize}
\tightlist
\item
  The larger the population effect size, the more Glass' \(d\) will overestimate \(\delta_{Glass}\);
\end{itemize}
...
\caption{\label{fig:biasGlassctrlsize2}Degrees of freedom (DF) and \(\gamma\), when computing the bias of Glass' \(d\), when variances are equal across groups, as a function of \(n_c\) (top) and \(n_e\) (bottom).}
\end{figure}
...
\subsection{\texorpdfstring{Cohen's \(\bm{d^*}\) (see Table 2)}{Cohen's \textbackslash bm\{d\^{}*\} (see Table 2)}}\label{cohens-bmd-see-table-2}}
...
\hypertarget{shiehs-bmd-see-table-2}{%
\subsection{\texorpdfstring{Shieh's \(\bm{d}\) (see Table 2)}{Shieh's \textbackslash bm\{d\} (see Table 2)}}\label{shiehs-bmd-see-table-2}}
...
The \textbf{bias} of Glass' \(d\) is a function of the population effect size (\(\delta_{Glass}\)) and the size of the control group (\(n_e\)):

\begin{itemize}
\tightlist
\item
  When \(\delta_{Glass}\) is null, the bias is null. In all other configurations, the larger \(\delta_{Glass}\), the more Glass' \(d\) will overestimate \(\delta_{Glass}\);\\
\item
