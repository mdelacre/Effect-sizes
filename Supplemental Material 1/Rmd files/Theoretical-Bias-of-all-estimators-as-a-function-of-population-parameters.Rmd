...
## Cohen's $\bm{d}$ (see Table 1)

Under the assumptions that independent residuals are normally distributed with equal variances, the **bias** of Cohen's $d$ is a function of total sample size ($N$) and the population effect size ($\delta_{Cohen}$):     
...
The **bias** of Glass' $d$ is a function of the control group size ($n_c$) and the population effect size ($\delta_{Glass}$):  
...
```{r biasGlassctrlsize2,fig.cap="Degrees of freedom (DF) and $\\gamma$, when computing the bias of Glass' $d$, when variances are equal across groups, as a function of $n_c$ (top) and $n_e$ (bottom).",echo=FALSE}
par(mfrow=c(2,2))
...
The **bias** of Glass' $d$ is a function of the population effect size ($\delta_{Glass}$) and the size of the control group ($n_e$):  
