...
The \textbf{sample mean difference, divided by the non pooled average of both variance estimates} (A) was suggested by {[}Cohen\_1988{]}. We immediately exclude this alternative because it suffers of many limitations:\\
...
Because the SD of the experimental group has no impact on the computed Glass's \(d_s\), one could advice to report both mean differences divided by the SD of the control group and mean differences divided by the SD of the experimental groups. However, it could induces large ambiguity because both measures could be substantially different (Shieh, 2013). When reporting only the mean difference divided by the SD of the control group, the measure is very stable from one design to another (e.g.~Glass's delta) but have weak inferential properties (as we will illustrate in the simulation section). While we consider that all purposes are important, the inferential purpose is crucial in the setting we are studying. We therefore consider this measure to be unsuitable.
...
Fortunately, this paradox can be resolved in answering the following question: \enquote{for any sample sizes ratio, what value of Shieh's \(d_s\) would have been computed if design were balanced (i.e.~\(n_1 = n_2\)), keeping constant the mean difference, \(SD_1\) and \(SD_2\)?} It can be shown that the relationship between the Shieh's \(d_s\) value when samples sizes are equal between groups (i.e.~\(\delta_{Shieh, n1=n2}\)) and Shieh's \(d_s\) values for all other sample sizes allocation ratios can be expressed as follows:
