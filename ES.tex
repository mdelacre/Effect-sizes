...
\emph{Note}. Cohen's \(d_s\) is a biased estimator, because its expectation differ from the population effect size. On the other hand, Hedge's \(g_s\) is an unbiased estimator, because its expectation equals the population effect size; equations in Table 1 require \(df \ge 3\) (i.e.~\(N \ge 5\)).
...
In his review, Shieh (2013) mentions three options available in the literature to deal with the case of unequal variances: the sample mean difference divided by (A) the Glass's \(d_s\), (B) the non pooled average of both variance estimates and (C) the Shieh's \(d_s\).
...
\hypertarget{glasss-g_s-cohens-g_s-and-shiehs-g_s}{%
\subsubsection{\texorpdfstring{Glass's \(g_s\), Cohen's \(g'_s\) and Shieh's \(g_s\)}{Glass's g\_s, Cohen's g'\_s and Shieh's g\_s}}\label{glasss-g_s-cohens-g_s-and-shiehs-g_s}}
...
\begin{longtable}[]{@{}lccc@{}}
\caption{Theoretical bias and variance of unbiased estimators.}\tabularnewline
\toprule
...
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.06\columnwidth}\raggedright
\tiny\(Cohen's \; g'_s\)\strut
\end{minipage} & \begin{minipage}[t]{0.13\columnwidth}\centering
/\strut
\end{minipage} & \begin{minipage}[t]{0.35\columnwidth}\centering
\tiny\(Var(Cohen's \; d'_s) \times \left[\frac{\Gamma(\frac{df}{2})}{\sqrt{\frac{df}{2}} \times \Gamma(\frac{df-1}{2})}\right]^2\)\strut
\end{minipage} & \begin{minipage}[t]{0.35\columnwidth}\centering
\tiny\(\frac{(n_1-1)(n_2-1)(s^2_1+s^2_2)^2}{(n_2-1)s^4_1+(n_1-1)s^4_2}\)\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.06\columnwidth}\raggedright
\strut
\end{minipage} & \begin{minipage}[t]{0.13\columnwidth}\centering
...
/\strut
\end{minipage} & \begin{minipage}[t]{0.35\columnwidth}\centering
\tiny\(Var(Shieh's \; d_s) \times \left[\frac{\Gamma(\frac{df}{2})}{\sqrt{\frac{df}{2}} \times \Gamma(\frac{df-1}{2})}\right]^2\)\strut
\end{minipage} & \begin{minipage}[t]{0.35\columnwidth}\centering
\tiny\(\approx \frac{\left(\frac{\sigma^2_1}{n_1}+\frac{\sigma^2_2}{n_2} \right)^2}{\frac{(\sigma^2_1/n_1)^2}{n_1-1}+\frac{(\sigma^2_2/n_2)^2}{n_2-1}}\)\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}

\emph{Note}. Equations for bias and variance of Cohen's \(d_s\) require that i.i.d. residuals are normally distributed (\(\epsilon_i \sim i.i.d.\; N(0,\sigma^2)\)) with equal variances across gruops (\(\sigma_1=\sigma_2\)). Equations for bias and variance of all other estimators require only that i.i.d. residuals are normally distributed.
