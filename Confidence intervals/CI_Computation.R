...
Cohen.CI <- function(Group.1, Group.2)
{ 
  n1 <- length(Group.1)
  n2 <- length(Group.2)
  ...
  # Limit of lambda such as pt(q=t_obs, df=n1+n2-2, ncp = lambda) = 0.025
  f=function(lambda,rep) pt(q=t_obs, df=n1+n2-2, ncp = lambda)-rep
  out=uniroot(f,c(-100,100),rep=.025)
  lambda.1 <- out$root
  ...
  # Limit of lambda such as 1-pt(q=t_obs, df=n1+n2-2, ncp = lambda) = 0.025
