...
# distribution théorique attendue pour le d cohen avec variances non poolées
...
#    theo_cohendprime <- sqrt((sd1^2/n1+sd2^2/n2)*(sd1^2+sd2^2)/2)*rt(1000000,df=df_t,ncp=ncp_t)    
...
for (i in 2){   # i = 2 = le dossier contenant les simulations de distributions normales
  ...
  res<-matrix(0,length(list.files(Folder[i])),length(col.res)+3) # +3 for mean diff, nratio and sample ratio 
...
for (i in 2){   # i = 2 = le dossier contenant les simulations de distributions normales
  ...
  for (j in seq_len(length(list.files(Folder[i])))){ 
    ...
    # bias 
    res[j,62] <-  mean(file[,18]) - cohen_deltaprime
    res[j,63] <-  COHEN_DELTAPRIME- cohen_deltaprime
    
    #relative bias
    res[j,64] <-  (mean(file[,18]) - cohen_deltaprime)/cohen_deltaprime
    res[j,65] <- (COHEN_DELTAPRIME - cohen_deltaprime)/cohen_deltaprime
