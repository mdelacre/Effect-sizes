...
```{r biasGlassctrlsize2,fig.cap="Degrees of freedom (DF) and $\\gamma$, when computing the bias of Glass's $d_s$, when variances are equal across groups, $n_c$ (top) and $n_e$ (bottom).",echo=FALSE}
par(mfrow=c(2,2))
plot(Nctrl,DF,xlab="N")
plot(Nctrl,coeffmult,ylab=expression(gamma),xlab="N")
plot(Nexp,DF2,xlab="N")
plot(Nexp,coeffmult2,ylab=expression(gamma),xlab="N")
```
...
par(mfrow=c(2,2))
plot(Nsize2,DF2,ylab="DF",xlab=expression(n[1]))
plot(Nsize4,DF4,ylab="DF",xlab=expression(n[1]))
plot(Nsize,DF,xlab=expression(n[2]))
plot(Nsize3,DF3,ylab="DF",xlab=expression(n[2]))
...
The **bias** of Shieh's $d'_s$ is a function of the population effect size ($\delta_{Shieh}$), the total sample size, and the interaction between the sample sizes ratio ($\frac{n_1}{n_2}$) and the $SD$-ratio :   
  ...
  + The smallest bias always occure when there is a positive pairing between $\frac{\sigma_1}{\sigma_1}$ and $\frac{n_1}{n_1}$. Moreover, the larger the $SD$-ratio, the further from 1 is the sample sizes ratio associated with the smallest bias (for more details, see "Theoretical Bias, as a function of population parameters").    
